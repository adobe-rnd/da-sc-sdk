...
- **`removeItem(pointer)`** — `applyRemove` splices out `pointer`'s item. Skipped only when the array is `readonly`; removal is never floored at `minItems`. Dropping below `minItems` — or emptying a required array — is surfaced as a validation error, not blocked.
